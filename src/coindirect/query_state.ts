...
export class QueryState {
  ...
  public get prevUrl(): string | null {
    ...
    if (prevOffset > -1) {
      const prevQuery = new QueryState(this.limit, prevOffset, this.sort.field, this.currency);
  ...
  public get nextUrl(): string | null {
    if (this.end < this.total) {
      const prevQuery = new QueryState(this.limit, this.end, this.sort.field, this.currency);
      const urlBuilder = new UrlBuilder(prevQuery, this.sort);
      return urlBuilder.toString();
    }
