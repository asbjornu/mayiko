...
import { describe, suit } from "./test_helper.ts";
...
suit(() => {
  describe("/", (x) => {
    ...
    it("should contain EUR currency", () =>
      x.expect(".currencies .eur a").toEqual((e) => e.textContent, "EUR"));
  });
  ...
  describe("/eur", (x) => {
    ...
    it("should return HTML", () =>
      x.expect("Content-Type", "text/html; charset=utf-8"));
    it("EUR should be chosen", () =>
      x.expect(".currencies .eur.chosen a").toEqual(
        (e) => e.textContent,
        "EUR",
      ));
    it("should include Austria", () =>
      x.expect(".countries .at .name").toEqual(
        (e) => e.textContent,
        "Austria",
      ));
  });
  ...
  describe("/eur?offset=20&limit=5", (x) => {
    ...
    it("should include Mayotte", () =>
      x.expect(".countries .yt .name").toEqual(
        (e) => e.textContent,
        "Mayotte",
      ));
  });
